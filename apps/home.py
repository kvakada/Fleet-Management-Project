...
df_cost_data = df_cost_data.round(decimals=2)
# df_vehicle_data = pd.read_csv("Graph Goals.csv")
...
y_data_delivery = np.random.randint(400, 451, 900)
...
fig_goals.update_layout(
    ...
    xaxis=dict(
        ...
        tickfont=dict(
            family='Arial',
            size=14,  # Replaces xaxis_tickfont_size
            color='rgb(82, 82, 82)',
        ),
    ),
    yaxis=dict(
...
fig_carbon.update_layout(
    ...
    yaxis=dict(
    title=dict(
        text='kept delivery dates/month',
        font=dict(size=16)
    ),
    tickfont=dict(size=14),
    showgrid=False,
...
fig_delivery_dates.add_trace(go.Scatter(
    x=x_data_delivery,
    y=y_data_kept_delivery[0],  # Only one line, index 0
    mode='lines+markers',
...
# Ensure there's a 'month' column extracted from a 'date' field
if 'month' not in df_cost_data.columns:
    df_cost_data['month'] = pd.to_datetime(df_cost_data['date']).dt.strftime('%B')

# Sum costs per month
monthly_costs = df_cost_data.groupby('month').sum(numeric_only=True)

# Use consistent month name casing and handle missing months gracefully
cost_juli = monthly_costs.loc['July'] if 'July' in monthly_costs.index else pd.Series(dtype='float64')
cost_juni = monthly_costs.loc['June'] if 'June' in monthly_costs.index else pd.Series(dtype='float64')

# Round values
cost_juli = cost_juli.round(2)
cost_juni = cost_juni.round(2)

# Cost change computation
if not cost_juli.empty and not cost_juni.empty and cost_juni['total_cost'] != 0:
    cost_change = ((cost_juli['total_cost'] / cost_juni['total_cost']) - 1) * 100
    cost_change = round(cost_change, 2)
else:
    cost_change = 0.0

# Vehicle stats
df_vehicle_active = values.loc[['delayed', 'idle', 'on time']].sum()
df_vehicle_total = values.sum()

# Availability rate
availability_categories = ['accident', 'delayed', 'idle', 'maintenance', 'on time']
available_vehicles = values.loc[availability_categories].sum()
availability_rate = round((1 - (available_vehicles / df_vehicle_total)) * 100, 2) if df_vehicle_total != 0 else 0.0

# Profit and Revenue
profit = 100000
revenue = cost_juli.get('total_cost', 0) + profit
