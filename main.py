...
# Merge vehicle info with fleet to get 'pid'
df_vehicle = df_vehicle_data[['vid', 'licence_plate', 'vehicle_class', 'vocation', 
                              'vehicle_type', 'fuel_type', 'drivetrain_type']].copy()
df_vehicle = pd.merge(df_vehicle, df_driving_data[['vid', 'pid']].drop_duplicates(), how='left', on='vid')

# Prepare vehicle class grouping
...
# Merge to get driver name
df_driver = pd.merge(df_vehicle, dfnames[['pid', 'last_name']], how='left', on='pid')
df_driver = df_driver.drop_duplicates()
df_driver = df_driver.drop(columns=['pid', 'drivetrain_type'], errors='ignore')  # Optional: drop unneeded cols

# Group by license plate and driver
df_group_driver = df_driver.groupby(['licence_plate', 'last_name']).size().reset_index(name='Amount')
...
app.layout = html.Div([
    ...
    html.Div(
        [
            html.Span([
                html.A([
                    html.Img(src=app.get_asset_url('fleetboard_logo.png'), style={'height': '36px'}),
